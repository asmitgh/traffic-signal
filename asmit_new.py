...
baseline_time = 5  # Default green light duration
...
with open(log_file_path, 'w', newline='') as log_file:
    ...
    log_writer.writerow(['Cycle', 'Main Cycle', 'Lane', 'Vehicle Count', 'Signal', 'Green Light Time'])

# Define global variables for simulation control
...
def update_status(cycle, main_cycle, lane, count, signal, green_time):
    ...
    with open(log_file_path, 'a', newline='') as log_file:
        log_writer = csv.writer(log_file)
        log_writer.writerow([cycle, main_cycle, lane + 1, count, signal, green_time])

# Main function to process lanes and update the simulation
def process_lanes():
    global simulation_running, cycle, main_cycle
    ...
    while simulation_running:
        ...
        if total_count > 0:
            # Compute dynamic green light durations based on vehicle density
            green_light_durations = [
                baseline_time + (count / total_count) * (baseline_time * 5)  # Max duration is baseline_time * 6
                for count in vehicle_counts
            ]
        else:
            # If no vehicles detected, use baseline duration
            green_light_durations = [baseline_time] * 4

        # Determine which lane to activate
        current_lane = (cycle - 1) % 4

        # Update status for the current lane
        update_status(cycle, main_cycle, current_lane, vehicle_counts[current_lane], "Green", green_light_durations[current_lane])
        print(f"Main Cycle {main_cycle}: Cycle {cycle} | Lane {current_lane + 1} Green Light - {green_light_durations[current_lane]:.2f} sec | Vehicle Counts: {vehicle_counts}")

        start_time = time.time()

        while simulation_running and (time.time() - start_time < green_light_durations[current_lane]):
            update_simulation_window(
                ["Green" if i == current_lane else "Red" for i in range(4)],
                green_light_durations,
                current_lane,
                green_light_durations[current_lane] - (time.time() - start_time),
                last_active_times
            )
...
def start_simulation():
    ...
    if not simulation_running:
        ...
        cycle = 1
        main_cycle = 1
        threading.Thread(target=process_lanes, daemon=True).start()
...
button_frame = ttk.Frame(root, padding=(20, 10), style="primary.TFrame")
button_frame.grid(row=1, column=0, sticky=NSEW)

ttk.Button(button_frame, text="Start Simulation", command=start_simulation, style="success.TButton").grid(row=0, column=0, padx=10, pady=10)
ttk.Button(button_frame, text="Pause Simulation", command=pause_simulation, style="warning.TButton").grid(row=0, column=1, padx=10, pady=10)
ttk.Button(button_frame, text="Stop Simulation", command=stop_simulation, style="danger.TButton").grid(row=0, column=2, padx=10, pady=10)
